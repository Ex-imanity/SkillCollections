...
| `cross-agent-review` | 2.0.1 | 本机 Codex / ClaudeCode / Grok 互做跨代理评审：primary 保连续性与修复，reviewer 返回带证据的 verdict | 跨代理评审 handoff：任一 primary 请另一 peer 审计划/代码；防作者自漏 | 依赖本机 `claude`/`codex`/`grok` 中实际调用的 reviewer CLI；fail-closed、并发 round-cap、脱敏；readiness 看真实信封；不用于单 agent 自审或普通 code review |
