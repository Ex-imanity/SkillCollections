...
- Success envelope fields: `text` (review body), `sessionId` (provenance), `stopReason == "end_turn"` (completion), optional `usage` and `total_cost_usd`. Non-finite or omitted cost persists as `total_cost_usd: null`. A non-`end_turn` stop reason fails closed as `completion_failure` even when text contains a verdict token.
