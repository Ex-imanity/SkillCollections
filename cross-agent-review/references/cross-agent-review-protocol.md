...
- **Cost/latency audit**: record provider-reported `total_cost_usd` + wall time per gate (`log_cost`); absent or non-finite USD is `null`, not zero.
...
- Success envelope fields: `text` (review body), `sessionId` + `usage.{input_tokens,output_tokens}` (provenance, Codex-symmetric), `stopReason == "end_turn"` (completion), optional `total_cost_usd`. Non-finite or omitted cost persists as `total_cost_usd: null`. A non-`end_turn` stop reason fails closed as `completion_failure` even when text contains a verdict token.
