...
## 2026-07-31: Treat gate observation as a first-class primary responsibility
- **Decision:** Before every adapter invocation, the primary retains a trackable terminal session ID or parent adapter PID. `review_started`, an early terminal-tool return, absent stdout, and absent review output are all `in_progress` evidence, not completion. It polls only that original process and may validate final stdout and artifacts only after observed process exit or the configured timeout; a lost session reattaches via PID, and a process without either handle remains unobservable until timeout rather than being retried.
- **Reason:** In Codex thread `019fb275-9558-7000-af15-135224a9436b`, an early terminal-tool release was initially misread as gate exit. Continued observation of the same process later produced a valid ClaudeCode `REQUEST CHANGES` review. The adapter launched correctly; the primary's ambiguous lifecycle prompt caused the false failure.
- **Source:** Codex thread `019fb275-9558-7000-af15-135224a9436b` (latest gate records); `cross-agent-review/tests/test_review_limits.py::ReviewLimitTests::test_user_docs_require_a_trackable_gate_handle_before_closure`; source and installed `pytest` outputs on 2026-07-31.

## 2026-07-31: Strengthen runner-handle recovery and identity checks
- **Decision:** Prefer retaining a parent adapter PID whenever the host exposes one. After terminal-session loss, reattach through that recorded PID. Because PIDs can be reused, compare the recorded parent process start time or command identity where the host exposes either; an unavailable or mismatched comparison is `unobservable`, which waits through the configured timeout and never authorizes a replacement gate. Validate final stdout JSON before output or handoff.
- **Reason:** ClaudeCode review of the Phase 10 prompt change found the original test did not lock the reattachment/unobservable branches or final-output ordering, and noted PID reuse as a bounded-latency failure mode. These additions improve the primary's process identity evidence without inventing adapter-specific OS guarantees.
- **Source:** `Review/ByClaudeCode/2026-07-31-cross-agent-review-lifecycle-prompt-review.md` (P2-A, P3-B, P3-C, P3-D); `cross-agent-review/tests/test_review_limits.py::ReviewLimitTests::test_user_docs_require_a_trackable_gate_handle_before_closure`.
