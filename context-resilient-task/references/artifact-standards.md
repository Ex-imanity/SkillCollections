...
- Known detector limits (deliberate, to protect against the more dangerous direction — a false positive silently drops a legitimate registry row): a password containing non-ASCII characters, a bare password containing the table delimiter `|`, and a password made only of letters (`password: SuperSecretPass`) are not detected — an all-letter value cannot be told apart from prose such as "token authenticates the request", and a false positive would silently drop a legitimate registry row. `verify_mrs.py` warnings are a safety net, not a scanner.
