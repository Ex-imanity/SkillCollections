...
| 文件 | 职责 | 更新方式 |
|------|------|---------|
| `task_state.md` | 当前状态 source of truth | **原地修改**，禁止追加 |
| `plan.md` | 任务计划 + Plan Registry | 原地修改阶段状态 |
| `snapshot.md` | 最新检查点快照 | 事件触发覆写 |
...
`task_state.md` 中维护两个固定区块，是待办项的**唯一权威来源**：
...
- 基于事件触发 snapshot，不按时间间隔

**DON'T:**
- 依赖对话记忆恢复状态
- 在 `task_state.md` 末尾追加 `## 日期 状态更新` 段落
