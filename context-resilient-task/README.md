...
> **utils.md**：把“如何访问环境/工具/资源”与“发现了什么、决定了什么、交付了什么”分开。它是**所有资源的唯一注册点**，其他文件只按 ID 引用（如 `(res: R3)`），不复制链接。恢复和压缩摘要回放注册表时**不做近期裁剪**（区块 pinned），但每条 1800 / 整体 4000 字符的预算仍生效。详见下文「资源注册表」。
...
- **区块 pinned**：不走"只保留最近 N 段"的裁剪，任何区块都不会因为不够新而被丢弃。但每条 1800 字符 / 整体 4000 字符的预算依然生效 —— pinned 去掉的是"近期悬崖"，不是预算。
- **按行脱敏**：单行 `restricted` 只丢自己，不再连带整张表消失；未标级别的行在同区块存在 `restricted` 时按 fail-closed 丢弃。
- **凭据形态一律不出摘要**：无论标什么级别，含 `scheme://user:pass@host`、`?token=…`、`Bearer …` 的条目都不会被输出，且 `verify_mrs.py` 直接判 invalid —— 要求删除该值，而不只是不显示。其他 MRS 文档（`findings.md`、`progress.md`…）含凭据时告警，因为它们同样会被回放或提交。
...
它扫描 MRS 内所有文档里的指针，按类型分类，把"可达资源"排在"本地文件噪音"之前，标出**已不存在的本地路径**，并给每行附 `file:line` 出处。

草稿是机器猜的，所以一律写成 `Sensitivity: restricted`：**落盘但不进任何摘要**，直到人工补齐用途/访问方式并改成真实级别 —— 机器猜测不该把未分类资源直接发布进"总是回放"的注册表。指针里带凭据的会先脱敏成 `user:***@` 再写入，并在输出中点名**仍持有原始值的源文件**。
